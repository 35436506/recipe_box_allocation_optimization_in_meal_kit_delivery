...
def export_to_excel(days, real_orders_proportions, wmape_site_values_bb, wmape_site_values_greedy, wmape_global_values, wmape_site_vs_final_bb, wmape_global_vs_final):
    ...
    wb.save("F1 capacity changes (B&B).xlsx")
...
for day in [-11, -10, -9]:
    factory_capacities = get_factory_capacities(day)
    allocation = allocations_bb[day]['allocation']
    orders = allocations_bb[day]['orders']
    
    # Convert allocation from order IDs to order objects
    allocation_objects = {factory: [next(o for o in orders if o['id'] == order_id) for order_id in order_ids] 
                          for factory, order_ids in allocation.items()}
    
    plot_allocation(allocation_objects, f"Allocation for LD{-day}", factory_capacities)
    
    # Print order information for each day
    print(f"\nOrder information for LD{-day}:")
    for factory, order_ids in allocation.items():
        real_orders = sum(1 for order_id in order_ids if next(o for o in orders if o['id'] == order_id)['is_real'])
