...
def generate_orders_for_day(real_proportion, simulated_proportion, existing_real_orders=None):
    ...
    while len(orders) < total_orders:
        ...
        # Assign a new ID only if it's a new order
        if 'id' not in new_order:
            new_order['id'] = max([o['id'] for o in orders if 'id' in o] + [0]) + 1
...
def run_allocation_process_over_time(start_day, end_day, total_orders):
    ...
    for day in range(start_day, end_day + 1):
        ...
        # Unpack the tuple returned by generate_orders_for_day
...
def plot_wmape_comparison(days, wmape_site_tabu, wmape_site_ID_based, wmape_global_tabu):
    ...
    plt.ylabel('WMAPE')
    plt.title('WMAPE comparison: TS vs ID-based (With order changes)', pad=20, fontsize=14)
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.show()
...
export_wmape_data_to_excel("TS vs ID-based (order changes).xlsx", days, wmape_site_tabu, wmape_site_ID_based, wmape_global_tabu)