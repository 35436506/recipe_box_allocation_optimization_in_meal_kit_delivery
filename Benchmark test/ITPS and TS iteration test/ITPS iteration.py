...
def plot_iteration_results(results, initial_wmape):
    iterations, wmape_values, times = zip(*results)
    
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 10), sharex=True)
    
    ax1.plot(iterations, wmape_values, marker='o')
...
plot_iteration_results(results, initial_wmape_site)

# Find the best result
best_result = min(results, key=lambda x: x[1])
best_iterations, best_wmape_site, best_time = best_result

# Calculate final WMAPE global using the best allocation
best_allocation, _ = iterative_targeted_pairwise_swap(
    allocation_day_t_minus_1, 
    allocation_day_t, 
    factory_capacities, 
    max_iterations=best_iterations
)
wmape_global = calculate_wmape_global(allocation_day_t_minus_1, best_allocation)
