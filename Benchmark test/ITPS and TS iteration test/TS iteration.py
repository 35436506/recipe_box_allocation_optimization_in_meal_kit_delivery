...
def plot_iteration_results(results):
    iterations, wmape_values, times = zip(*results)
    
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 10), sharex=True)
    
    # Plot WMAPE site vs. Number of iterations
    ax1.plot(iterations, [round(wmape, 4) for wmape in wmape_values], marker='o')
    ax1.set_ylabel('WMAPE site')
    ax1.set_title('WMAPE site vs. Number of iterations (TS)')
    ax1.grid(True)
    
    # Set y-axis of the first plot to display 4 decimal places
    ax1.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, p: f"{x:.4f}"))
    
    # Plot Optimization time vs. Number of iterations
...
initial_wmape_site = calculate_wmape_site(allocation_t_minus_1, allocation_t)
...
plot_iteration_results(results)

# Find the best result
best_result = min(results, key=lambda x: x[1])
best_iterations, best_wmape_site, best_time = best_result

# Calculate WMAPE global using the best allocation
best_allocation, _ = tabu_search(
    allocation_t_minus_1, 
    allocation_t, 
    factory_capacities, 
    max_iterations=best_iterations
)
wmape_global = calculate_wmape_global(allocation_t_minus_1, best_allocation)

print(f"Initial WMAPE site: {initial_wmape_site:.4f}")
print(f"\nBest WMAPE site: {best_wmape_site:.4f} (achieved with {best_iterations} iterations)")
...
# Print all results
print("{:^10} | {:^10} | {:^15}".format("Iterations", "WMAPE site", "Time (seconds)"))
print("-" * 40)
for iterations, wmape, time_taken in results:
    print("{:^10d} | {:^10.4f} | {:^15.2f}".format(iterations, wmape, time_taken))