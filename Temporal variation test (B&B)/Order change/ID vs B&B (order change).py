...
def generate_orders_for_day(real_proportion, simulated_proportion, existing_real_orders=None):
    ...
    if existing_real_orders:
        # Delete 15% of real orders
        delete_count = int(0.15 * len(existing_real_orders))
        ...
        # Change 10% of remaining real orders
        change_count = int(0.1 * len(remaining_orders))
...
def allocate_orders_ID_based(orders, factory_capacities, previous_allocation, changed_orders, deleted_orders):
    ...
    # First, allocate unchanged real orders to their previous factories
...
def plot_wmape_comparison(days, wmape_site_bb, wmape_site_ID_based, wmape_global_bb):
    ...
    plt.ylabel('WMAPE')
    plt.title('WMAPE comparison: B&B vs ID-based (With order change)', pad=20, fontsize=14)
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.show()
...
export_wmape_data_to_excel("ID vs BB (order change).xlsx", days, wmape_site_bb, wmape_site_ID_based, wmape_global_bb)