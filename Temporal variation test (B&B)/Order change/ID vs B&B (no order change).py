...
def plot_wmape_comparison(days, wmape_site_bb, wmape_site_ID_based, wmape_global_bb):
    ...
    plt.ylabel('WMAPE')
    plt.title('WMAPE comparison: B&B vs ID-based (Without order change)', pad=20, fontsize=14)
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.show()
...
export_wmape_data_to_excel("ID vs BB (no order change).xlsx", days, wmape_site_bb, wmape_site_ID_based, wmape_global_bb)